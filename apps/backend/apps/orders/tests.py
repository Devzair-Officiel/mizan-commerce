...
def setup(email):
    ...
    product = Product.objects.create(shop=shop, name='Article test', selling_price='20.00')
    StockMovement.objects.create(shop=shop, product=product, movement_type='in', quantity=50, created_by=user)
    customer = Customer.objects.create(shop=shop, name='Client test')
    return user, shop, product, customer


class OrderServiceTest(TestCase):

    def setUp(self):
        self.user, self.shop, self.product, self.customer = setup('service@example.com')
    ...
    def test_add_item_calculates_total(self):
        order = services.create_order(self.shop, self.user)
        services.add_item(order, self.product, 3)
    ...
    def test_total_with_discount_and_shipping(self):
        ...
        services.add_item(order, self.product, 2)
        order.refresh_from_db()
        # 40 - 5 + 3 = 38
    ...
    def test_transition_to_prepare_reserves_stock(self):
        order = services.create_order(self.shop, self.user)
        services.add_item(order, self.product, 5)
        services.transition_status(order, 'to_prepare', self.user)
        self.product.refresh_from_db()
        self.assertEqual(self.product.stock_quantity, 45)  # 50 - 5
    ...
    def test_cancel_releases_stock(self):
        ...
        self.product.refresh_from_db()
        self.assertEqual(self.product.stock_quantity, 50)  # restauré
    ...
    def test_payment_status_updates(self):
        order = services.create_order(self.shop, self.user)
        services.add_item(order, self.product, 2)  # total = 40
        order.refresh_from_db()
...
class OrderAPITest(TestCase):
    ...
    def setUp(self):
        ...
        self.user, self.shop, self.product, self.customer = setup('api@example.com')
...
class OrderMultiTenantTest(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user_a, self.shop_a, self.product_a, _ = setup('a@example.com')
        self.user_b, self.shop_b, self.product_b, _ = setup('b@example.com')
